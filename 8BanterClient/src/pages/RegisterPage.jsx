...
const RegisterPage = () => {
  ...
  const handleRegister = async () => {
    try {
      const response = await serverApi.post("users/register", {
        username,
        email,
        password,
      });
  ...
  const handleGoogleLoginSuccess = async (googleResponse) => {
    try {
      ...
      const response = await serverApi.post("/auth/google", { id_token: credential });
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="max-w-md mx-auto bg-primary p-6 rounded-lg shadow-lg">
        ...
        <div className="text-center">
          <p className="text-sm mb-2">Or register with Google</p>
          <button>
            <GoogleOAuthProvider
              clientId={
